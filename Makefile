...
HTTPS_GIT := github.com/reece/myproject.git
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=myproject \
...
ifeq ($(OS),Windows_NT)
	...
	go build -mod=readonly $(BUILD_FLAGS) -o build/appd ./cmd/appd
endif

build-windows-client: go.sum
	GOOS=windows GOARCH=amd64 go build -mod=readonly $(BUILD_FLAGS) -o build/appd.exe ./cmd/appd
...
draw-deps:
	...
	@goviz -i ./cmd/appd -d 2 | dot -Tpng -o dependency-graph.png
...
ifeq (,$(shell which heighliner))
	...
	heighliner build -c myproject --local -f chains.yaml
endif
...
set-testnet-configs:
	appd config set client chain-id localchain-1
	appd config set client keyring-backend test
	appd config set client output text

# import keys from testnet.json into test keyring
setup-testnet-keys:
	-`echo "decorate bright ozone fork gallery riot bus exhaust worth way bone indoor calm squirrel merry zero scheme cotton until shop any excess stage laundry" | appd keys add acc0 --recover`
	-`echo "wealth flavor believe regret funny network recall kiss grape useless pepper cram hint member few certain unveil rather brick bargain curious require crowd raise" | appd keys add acc1 --recover`
